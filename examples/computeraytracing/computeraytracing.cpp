...
public:
	...
	{		
		...
		{
			// LOOKAT 2.3.2: release the storage image from graphics queue
			...
			imageMemoryBarrier.dstAccessMask = 0; // don't care
			...
			vkCmdPipelineBarrier(
				...
				VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, // don't care
				VK_FLAGS_NONE,
				0, nullptr,
				0, nullptr,
		...
		// LOOKAT 2.6.1 TODO 2.6: change magFilter to linear
		// Create sampler
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			if (vulkanDevice->queueFamilyIndices.graphics != vulkanDevice->queueFamilyIndices.compute)
			{
				// TODO 6.3.2: acquire from graphics queue, set the dst stage and dst access
			...
			// TODO 5.2.2 bind graphics.descriptorSet, bind pipeline, draw one triangle
	...
	void buildComputeCommandBuffer()
	{
		...
		if (vulkanDevice->queueFamilyIndices.graphics != vulkanDevice->queueFamilyIndices.compute)
		{
			// TODO 6.3.1: release from compute queue, set the src stage and src access
	...
	void prepareStorageBuffers()
	{
		...
		// TODO 2.5.1: create a host staging buffer
		...
		// TODO 2.5.2: create a device local buffer
	...
	{
		// LOOKAT 3.2 : This the total number and type of descriptors that we are going to use
	...
	void prepareGraphics()
	{
		...
		// TODO 4.1: choose the correct polygon mode
	...
	void prepareCompute()
	{
		...
		// Setup descriptors
		// TODO 3.1: create LayoutBinding for binding 1 and binding 2
		...
		// TODO 3.2: write descriptor set for binding 1 and binding 2
		...
		if (vulkanDevice->queueFamilyIndices.graphics != vulkanDevice->queueFamilyIndices.compute)
		{
			...
			VK_CHECK_RESULT(vkCreateSemaphore(vulkanDevice->logicalDevice, &semaphoreCreateInfo, VK_NULL_HANDLE, &compute.semaphore));
		}
	...
	void draw()
	{
		// TODO 6.1.1: wait for compute.fence and reset it
		...
		// TODO 6.1.2: submit the compute command buffer, signal compute.fence while it finishes
		...
		// LOOKAT 6.2.1: wait for next image to be available from the swapchain
		// TODO 6.4.1: also wait for the compute job to finish
		std::vector<VkSemaphore> graphicsWaitSemaphores = {semaphores.presentComplete, compute.semaphore};
		graphicsSubmitInfo.waitSemaphoreCount = vulkanDevice->queueFamilyIndices.graphics != vulkanDevice->queueFamilyIndices.compute ? graphicsWaitSemaphores.size() : 1;
		graphicsSubmitInfo.pWaitSemaphores = graphicsWaitSemaphores.data();
		// LOOKAT 6.2.2
		// TODO 6.4.2: set the wait stage for compute semaphore
		std::vector<VkPipelineStageFlags> graphicsSubmitPipelineFlags = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT };
		graphicsSubmitInfo.pWaitDstStageMask = graphicsSubmitPipelineFlags.data();
